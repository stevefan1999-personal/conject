//! Integration tests combining multiple nject features together.
#![allow(dead_code)]

use nject::{init, injectable, key, module, provider, Factory, Late, Lazy, Named};
use std::sync::Arc;
...
fn circular_deps_with_late() {
    ...
    assert!(Arc::ptr_eq(&bus.subscribers.bus, &bus));
