...
pub(crate) fn handle_module(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    ...
    let struct_export_outputs = struct_exports_by_type
        ...
        .map(|exports| {
            ...
            let iter_output = quote! {
                impl<'prov, #(#generic_params,)*ConjectProvider> ::conject::RefIterable<'prov, #ty, ConjectProvider> for #ident<#(#generic_keys),*>
                    where
                        #prov_lifetimes
                        ConjectProvider: #(::conject::Provider<'prov, #prov_types>)+*, #where_predicates
                {
                    #[inline(always)]
                    fn inject(&'prov self, provider: &'prov ConjectProvider, index: usize) -> #ty {
                        match index {
                            #( #iter_match_outputs )*
            ...
            quote! {
                ...
                impl<'prov, #(#generic_params,)*ConjectProvider> ::conject::RefInjectable<'prov, #ty, ConjectProvider> for #ident<#(#generic_keys),*>
                    where
                        #prov_lifetimes
                        ConjectProvider: #(::conject::Provider<'prov, #prov_types>)+*, #where_predicates
                {
                    #[inline(always)]
                    fn inject(&'prov self, provider: &'prov ConjectProvider) -> #ty {
                        #(let #inputs = provider.provide();)*
                        #value
                    }
                }
    ...
    let export_outputs = export_attr_indexes.iter().map(|(i, attrs)| {
        ...
        let outputs = inputs.map(|input| {
            ...
            quote! {
                ...
                impl<'prov, #(#generic_params,)*ConjectProvider> ::conject::Injectable<'prov, #ty, ConjectProvider> for #ty
                    where
                        #prov_lifetimes
                        ConjectProvider: ::conject::Import<#ident<#(#generic_keys),*>>, #where_predicates
                {
                    #[inline(always)]
                    fn inject(provider: &'prov ConjectProvider) -> #ty {
                        #body
                    }
                }
            }
    ...
    let output = quote! {
        #[derive(::conject::ModuleHelperAttr)]
        #input
