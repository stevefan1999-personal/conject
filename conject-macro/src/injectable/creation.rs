...
pub(crate) fn field_init_expr(ty: &Type, attr: &Option<InjectExpr>) -> proc_macro2::TokenStream {
    match attr {
        Some(InjectExpr::Named(tag)) => {
            quote! { ::conject::Named::<#tag, #ty>::into_inner(provider.provide()) }
        }
        Some(InjectExpr::NamedStr(hash)) => {
            quote! { ::conject::Named::<::conject::Key<#hash>, #ty>::into_inner(provider.provide()) }
        }
        ...
        #[cfg(feature = "env")]
        Some(InjectExpr::Env(key, default)) => match default {
