...
use crate::module::{models::ModuleKey, repository};
...
pub(super) fn gen_imports_for_import_attr(
    ...
    let imported_modules = import_attr_indexes
        ...
        .map(|i| {
            let field = fields[*i];
            let ty = &field.ty;
            let import_key = ModuleKey::from(ty);
            let import = repository::get(&import_key);
    ...
    let import_iter_outputs = exported_types.values().map(|types| {
        ...
        let iter_match_outputs = types_by_module.values().flat_map(|types_for_module| {
            types_for_module.iter().enumerate().map(|(mod_index, (index, (_, ty, field_key)))| {
                quote! { #index => ::conject::RefIterable::<#ty, #ident<#(#generic_keys),*>>::inject(&self.provider.#fields_path_prefix #field_key, self.provider, #mod_index), }
            })
        });
        quote!{

            impl<'prov, #(#generic_params),*> ::conject::Iterable<'prov, #ty> for #ident<#(#generic_keys),*>
                where #where_predicates
            {
                #[inline(always)]
                fn iter(&'prov self) -> impl ::core::iter::Iterator<Item = #ty> {
                    struct ConjectIterator<'prov, #(#generic_params),*> {
                        provider: &'prov #ident<#(#generic_keys),*>,
                        index: usize,
                    }
                    impl<'prov, #(#generic_params),*> ::core::iter::Iterator for ConjectIterator<'prov, #(#generic_keys),*> {
                        type Item = #ty;
        ...
        quote!{
                ...
                where #where_predicates
            {
                ...
                fn provide(&'prov self) -> #ty {
                    ::conject::RefInjectable::<#ty, Self>::inject(&self.#fields_path_prefix #field_key, self)
                }
            }
        }
    });
    let import_impl_outputs = imported_modules.iter().map(|(_, field_key, field)| {
        ...
        quote! {
            ...
            impl<#(#generic_params),*> ::conject::Import<#ty_output> for #ident<#(#generic_keys),*>
                where #where_predicates
            {
                #[inline(always)]
